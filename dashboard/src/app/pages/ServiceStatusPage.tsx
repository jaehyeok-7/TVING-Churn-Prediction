...
const PieTooltip = ({ active, payload }: any) => {
  ...
  return (
    <div style={{ background: C.card, border: `1px solid ${d.color}40`, borderRadius: "8px", padding: "10px 14px" }}>
      <p style={{ fontFamily: "Pretendard, sans-serif", fontSize: "12px", fontWeight: 700, color: d.color, margin: "0 0 2px" }}>{d.name}</p>
      <p style={{ fontFamily: "Inter, sans-serif", fontSize: "14px", fontWeight: 700, color: C.text, margin: 0 }}>{d.pct}%</p>
    </div>
  );
};

export function ServiceStatusPage({ embedded = false }: { embedded?: boolean }) {
  const { periodLabel, isCustom } = usePeriodFilter();
  const { users = [], watchLogs = [], status } = useSheetData();

  const totalUsers = users.length;

  const { avgWatchTime, avgChurn, avgCompletion, avgLastActive, avgDivScore } = useMemo(() => {
    if (totalUsers === 0) return { avgWatchTime: 0, avgChurn: 0, avgCompletion: 0, avgLastActive: 0, avgDivScore: 0 };
  ...
  const KPI_DATA = [
    { label: "총 활성 사용자",    value: totalUsers.toLocaleString(), unit: "명",  delta: "+2.4%",  deltaDir: "up",   icon: Users,          color: C.green,  glow: "rgba(0,210,160,0.18)"  },
    { label: "평균 시청 시간",    value: avgWatchTime,                unit: "시간", delta: "+0.5h",  deltaDir: "up",   icon: Clock,          color: C.purple, glow: "rgba(108,99,255,0.18)" },
    { label: "평균 이탈 예측률",  value: avgChurn,                    unit: "%",    delta: "-1.2%p", deltaDir: "down", icon: AlertTriangle, color: C.red,    glow: "rgba(227,6,19,0.18)"   },
    { label: "평균 완주율",       value: avgCompletion,               unit: "%",    delta: "+3.1%p", deltaDir: "up",   icon: Play,           color: C.orange, glow: "rgba(255,183,77,0.18)" },
  ];
  ...
  const segmentData = useMemo(() => {
    ...
    return [
      { name: "정주행러 (Power)",   count: counts.Heavy,  pct: totalUsers ? Math.round((counts.Heavy  / totalUsers) * 100) : 0, color: C.purple },
      { name: "일상러 (Regular)",   count: counts.Medium, pct: totalUsers ? Math.round((counts.Medium / totalUsers) * 100) : 0, color: C.green  },
      { name: "찍먹러 (Light)",     count: counts.Light,  pct: totalUsers ? Math.round((counts.Light  / totalUsers) * 100) : 0, color: C.orange },
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "14px" }}>
        {KPI_DATA.map((kpi) => {
          ...
          return (
            <div
              key={kpi.label}
              style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: "12px", padding: "20px", position: "relative", overflow: "hidden", transition: "border-color 0.2s" }}
              onMouseEnter={(e) => ((e.currentTarget as HTMLDivElement).style.borderColor = kpi.color + "55")}
            ...
            >
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "12px" }}>
                ...
                <p style={{ fontFamily: "Pretendard, sans-serif", fontSize: "12px", color: C.sub, margin: 0, fontWeight: 500 }}>{kpi.label}</p>
              </div>
              <div style={{ display: "flex", alignItems: "baseline", gap: "4px", marginBottom: "8px" }}>
                <span style={{ fontFamily: "Inter, sans-serif", fontSize: "26px", fontWeight: 800, color: C.text, letterSpacing: "-0.5px", lineHeight: 1 }}>{kpi.value}</span>
                {kpi.unit && <span style={{ fontFamily: "Pretendard, sans-serif", fontSize: "14px", fontWeight: 500, color: kpi.color }}>{kpi.unit}</span>}
              </div>
        ...
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: "12px", padding: "22px" }}>
          ...
          <div style={{ position: "relative", height: 160 }}>
            <ResponsiveContainer width="100%" height={160}>
              <PieChart>
                <Pie data={segmentData} cx="50%" cy="50%" innerRadius={46} outerRadius={70} paddingAngle={3} dataKey="pct" startAngle={90} endAngle={450}>
                  {segmentData.map((d, i) => <Cell key={`seg-${i}`} fill={d.color} stroke="none" />)}
                </Pie>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", flex: 1 }}>
            ...
            <tbody>
              {healthRows.map((r, i) => {
                ...
                const stBg     = isOk ? "rgba(0,210,160,0.1)"   : "rgba(255,183,77,0.1)";
                const stBorder = isOk ? "rgba(0,210,160,0.25)"  : "rgba(255,183,77,0.25)";
                const stLabel  = isOk ? "✅ 정상" : "⚠️ 주의";
                return (
                  <tr key={r.label} style={{ borderBottom: i < healthRows.length - 1 ? `1px solid ${C.border}` : "none" }}
                  ...
                  >
                    ...
                    <td style={{ padding: "14px 20px" }}>
                      <span style={{ display: "inline-flex", alignItems: "center", padding: "3px 10px", borderRadius: "20px", background: stBg, border: `1px solid ${stBorder}`, fontFamily: "Pretendard, Inter, sans-serif", fontSize: "11px", fontWeight: 600, color: stColor }}>
                        {stLabel}
                      </span>
                    </td>
                  </tr>
